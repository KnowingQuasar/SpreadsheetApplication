...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        {
            //Linked list of items to change (in order)
            LinkedList<String> changed = new LinkedList<String>();
            //Set of names that have been visited
            HashSet<String> visited = new HashSet<String>();
            foreach (String name in names)
            {
                //If the name hasn't been visited, visit it.
                if (!visited.Contains(name))
                {
                    Visit(name, name, visited, changed);
                }
            }
            //Return ordered list of names of cells to recalculate
            return changed;
        }
        ...
        {
            //Make sure that we don't infintely visit the same name, add a name to the visited set
            visited.Add(name);
            //For each direct dependant of name:
            foreach (String n in GetDirectDependents(name))
            {
                //If name is a dependant of itself (i.e. if A is a dependant of A) throw a CircularException
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                //If the dependant hasn't already been visited, visit it. This assures that all dependencies are covered.
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            //Add the cell name to change to the changed linked list. This assures correct ordering when recalculating.
